...
public class BukkitApiImplementer implements Implementer {
    ...
    private static final String SUPPORT = "io/izzel/freshwaterfish/boot/asm/BukkitApiSupport";
    ...
    public boolean processClass(ClassNode node, ILaunchPluginService.ITransformerLoader transformerLoader) {
        ...
        if (changed) {
            FreshwaterFishImplementer.LOGGER.debug(MARKER, "Injected missing Bukkit API methods into {}", node.name);
        }
        return changed;
    }
}
