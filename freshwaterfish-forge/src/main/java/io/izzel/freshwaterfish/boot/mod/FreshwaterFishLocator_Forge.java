package io.izzel.freshwaterfish.boot.mod;
...
public class FreshwaterFishLocator_Forge implements IModLocator {

    private static final Set<String> EXCLUDES = Set.of(
            "net.minecraft.world.level.block"
    );
    private final IModFile freshwaterfish;

    public FreshwaterFishLocator_Forge() {
        ModBootstrap.run();
        this.freshwaterfish = loadJar();
    }

    @Override
    public List<ModFileOrException> scanMods() {
        FreshwaterFishJarInJarAdaptor.inject();
        return List.of(new ModFileOrException(freshwaterfish, null));
    }
    ...
    protected IModFile loadJar() {
        try {
            ...
            var version = System.getProperty("freshwaterfish.version");
            var path = Paths.get(".freshwaterfish", "mod_file", version + ".jar");
    ...
    private JarMetadata excludePackages(SecureJar secureJar, String version) {
        ...
        return new SimpleJarMetadata("freshwaterfish", version.substring(version.indexOf('-') + 1), secureJar.getPackages(), List.of());
    }
}
