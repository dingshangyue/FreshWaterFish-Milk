package io.izzel.freshwaterfish.boot;
...
public class AbstractBootstrap {
    ...
    protected void setupMod() throws Exception {
        ...
        var logger = LogManager.getLogger("FreshwaterFish");
        try (InputStream stream = getClass().getModule().getResourceAsStream("/META-INF/MANIFEST.MF")) {
            ...
            logger.info(FreshwaterFishLocale.getInstance().get("logo"),
                    FreshwaterFishLocale.getInstance().get("release-name." + ArclightVersion.current().getReleaseName()), version, buildTime);
        }
    }

    private void extract(InputStream path, String version) throws Exception {
        System.setProperty("freshwaterfish.version", version);
        var dir = Paths.get(".freshwaterfish", "mod_file");
        if (!Files.exists(dir)) {
            Files.createDirectories(dir);
        }
        var mod = dir.resolve(version + ".jar");
        if (!Files.exists(mod) || Boolean.getBoolean("freshwaterfish.alwaysExtract")) {
