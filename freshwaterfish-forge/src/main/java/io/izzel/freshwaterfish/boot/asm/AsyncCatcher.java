...
import io.izzel.freshwaterfish.i18n.conf.AsyncCatcherSpec;
...
public class AsyncCatcher implements Implementer {
    ...
    public AsyncCatcher() {
        ...
        this.defaultOp = FreshwaterFishConfig.spec().getAsyncCatcher().getDefaultOp();
        this.dump = FreshwaterFishConfig.spec().getAsyncCatcher().isDump();
        this.warn = FreshwaterFishConfig.spec().getAsyncCatcher().isWarn();
    ...
    public static <T> CallbackInfoReturnable<T> checkOp(Supplier<T> method, AsyncCatcherSpec.Operation operation, String reason, Executor executor) throws Throwable {
        if (INSTANCE.warn) {
            FreshwaterFishImplementer.LOGGER.warn(MARKER, "Async " + reason);
        }
        IllegalStateException exception = new IllegalStateException("Asynchronous " + reason + "!");
        if (INSTANCE.dump) {
            FreshwaterFishImplementer.LOGGER.debug(MARKER, "Async " + reason, exception);
        }
        switch (operation) {
            case NONE:
            ...
            case BLOCK: {
                ...
                try {
                    ...
                    FreshwaterFishImplementer.LOGGER.error(MARKER, "Async catcher timeout", ex);
                    throw e;
                }
                return cir;
            }
    ...
    private void injectCheck(ClassNode node, MethodNode methodNode, String reason) {
        FreshwaterFishImplementer.LOGGER.debug(MARKER, "Injecting {}/{}{} for reason {}", node.name, methodNode.name, methodNode.desc, reason);
        AsyncCatcherSpec.Operation operation = FreshwaterFishConfig.spec().getAsyncCatcher().getOverrides().getOrDefault(reason, defaultOp);
        InsnList insnList = new InsnList();
        LabelNode labelNode = new LabelNode(new Label());
        LabelNode labelNode1 = new LabelNode(new Label());
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/izzel/freshwaterfish/common/mod/server/FreshwaterFishServer", "isPrimaryThread", "()Z"));
        ...
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, "io/izzel/freshwaterfish/common/mod/server/FreshwaterFishServer", "getMainThreadExecutor", "()Ljava/util/concurrent/Executor;", false));
        insnList.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getType(AsyncCatcher.class).getInternalName(), "checkOp", "(Ljava/util/function/Supplier;Lio/izzel/freshwaterfish/i18n/conf/AsyncCatcherSpec$Operation;Ljava/lang/String;Ljava/util/concurrent/Executor;)Lorg/spongepowered/asm/mixin/injection/callback/CallbackInfoReturnable;"));
    ...
    private void instantiateCallback(ClassNode node, MethodNode methodNode, InsnList insnList) {
        ...
        FreshwaterFishImplementer.loadArgs(insnList, methodNode, methodType.getArgumentTypes(), 0);
    ...
    private String createImplType(ClassNode node, MethodNode methodNode, ClassNode classNode, MethodNode bridge) {
        ...
        FreshwaterFishImplementer.LOGGER.debug(MARKER, "Defined impl callback class {}", classNode.name);
        return init.desc;
    }
    ...
    private MethodNode createBridge(ClassNode node, MethodNode methodNode) {
        ...
        Type methodType = Type.getMethodType(methodNode.desc);
        FreshwaterFishImplementer.loadArgs(ret.instructions, methodNode, methodType.getArgumentTypes(), 0);
        ...
        FreshwaterFishImplementer.LOGGER.debug(MARKER, "Bridge method {}/{}{} created", node.name, ret.name, ret.desc);
        return ret;
    }
}
