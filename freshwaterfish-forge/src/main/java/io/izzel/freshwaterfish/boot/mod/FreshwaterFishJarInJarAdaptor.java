...
public class FreshwaterFishJarInJarAdaptor implements IDependencyLocator {
    ...
    public FreshwaterFishJarInJarAdaptor(IDependencyLocator delegate) {
        ...
        try {
            ...
            var newList = locatorList.stream().map(it -> {
                if (it instanceof JarInJarDependencyLocator) {
                    return new FreshwaterFishJarInJarAdaptor(it);
                } else {
                    return it;
                }
            }).toList();
    ...
    public List<IModFile> scanMods(Iterable<IModFile> loadedMods) {
        return delegate.scanMods(loadedMods).stream().filter(it -> {
            ...
            optional.ifPresent(module -> LOGGER.info("Skip jij dependency {}@{} because FreshwaterFish has {}",
