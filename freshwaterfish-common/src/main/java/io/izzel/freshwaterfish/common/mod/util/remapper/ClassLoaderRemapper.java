...
import io.izzel.freshwaterfish.common.mod.util.remapper.generated.FreshwaterFishReflectionHandler;
...
public class ClassLoaderRemapper extends LenientJarRemapper {
    ...
    private static final String REPLACED_NAME = Type.getInternalName(FreshwaterFishReflectionHandler.class);
    ...
    public ClassLoaderRemapper(JarMapping jarMapping, JarMapping toBukkitMapping, ClassLoader classLoader) {
        ...
        this.jarMapping.setInheritanceMap(FreshwaterFishRemapper.INSTANCE.inheritanceMap);
    ...
    private static byte[] dump(byte[] bytes) {
        try {
            if (FreshwaterFishRemapper.DUMP != null) {
                String className = new ClassReader(bytes).getClassName() + ".class";
                int index = className.lastIndexOf('/');
                if (index != -1) {
                    File file = new File(FreshwaterFishRemapper.DUMP, className.substring(0, index));
                    file.mkdirs();
                    Files.write(file.toPath().resolve(className.substring(index + 1)), bytes);
                } else {
                    Files.write(FreshwaterFishRemapper.DUMP.toPath().resolve(className), bytes);
                }
            }
        } catch (Exception e) {
    ...
    public Map.Entry<String, String> tryClimb(Map<String, String> map, String owner, String name, int access) {
        ...
        if (mapped == null && (access == -1 || (!Modifier.isPrivate(access) && !Modifier.isStatic(access)))) {
            ...
            if (FreshwaterFishRemapper.INSTANCE.inheritanceMap.hasParents(owner)) {
                parents = FreshwaterFishRemapper.INSTANCE.inheritanceMap.getParents(owner);
            } else {
                parents = GlobalClassRepo.inheritanceProvider().getParents(owner);
                FreshwaterFishRemapper.INSTANCE.inheritanceMap.setParents(owner, parents);
            }
    ...
    public Product2<byte[], CodeSource> remapClass(String className, Callable<byte[]> byteSource, URLConnection connection, FreshwaterFishRemapConfig config) throws ClassNotFoundException {
        try {
            FreshwaterFishClassCache.CacheSegment segment = FreshwaterFishClassCache.instance().makeSegment(connection);
            ...
            if (optional.isPresent()) {
                ...
                if (FreshwaterFishConfig.spec().getOptimization().isCachePluginClass()) {
                    ClassWriter cw = new ClassWriter(0);
    ...
    private byte[] remapClassFile(ClassReader reader, final ClassRepo repo) {
        ...
        FreshwaterFishRemapConfig config;
        if (repo instanceof ClassRepoWrapper wrapper) {
            config = wrapper.config();
        } else {
            FreshwaterFishMod.LOGGER.warn("classloader.no-remap-config", node.name.replace('/', '.'));
            config = FreshwaterFishRemapConfig.PLUGIN;
        }

        for (PluginTransformer transformer : FreshwaterFishRemapper.INSTANCE.getTransformerList()) {
    ...
    private Class<?> generateReflectionHandler() {
        try {
            ClassNode node = MixinService.getService().getBytecodeProvider().getClassNode(Type.getInternalName(FreshwaterFishReflectionHandler.class));
            Preconditions.checkNotNull(node, "node");
            ClassWriter writer = new ClassWriter(0);
            String name = Type.getInternalName(FreshwaterFishReflectionHandler.class) + "_" + COUNTER.getAndIncrement();
    ...
    private static class NameRemapper extends Remapper {
        ...
        private static final String ORIGIN = Type.getInternalName(FreshwaterFishReflectionHandler.class);
    ...
    private static class PluginClassWriter extends ClassWriter {
        ...
        private String getSuper(final String typeName) {
            ...
            return FreshwaterFishRemapper.getNmsMapper().map(node.superName);
        }
    }
