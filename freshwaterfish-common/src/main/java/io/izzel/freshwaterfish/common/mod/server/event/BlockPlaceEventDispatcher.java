...
import io.izzel.freshwaterfish.common.mod.util.FreshwaterFishBlockSnapshot;
...
public class BlockPlaceEventDispatcher {
    ...
    public void onBlockPlace(BlockEvent.EntityPlaceEvent event) {
        ...
        if (entity instanceof ServerPlayerEntityBridge playerEntity) {
            ...
            Direction direction = FreshwaterFishCaptures.getPlaceEventDirection();
            if (direction != null && DistValidate.isValid(event.getLevel())) {
                InteractionHand hand = FreshwaterFishCaptures.getPlaceEventHand(InteractionHand.MAIN_HAND);
                CraftBlock placedBlock = FreshwaterFishBlockSnapshot.fromBlockSnapshot(event.getBlockSnapshot(), true);
        ...
        if (entity instanceof ServerPlayerEntityBridge playerEntity) {
            ...
            if (direction != null && DistValidate.isValid(event.getLevel())) {
                ...
                for (BlockSnapshot snapshot : event.getReplacedBlockSnapshots()) {
                    placedBlocks.add(FreshwaterFishBlockSnapshot.fromBlockSnapshot(snapshot, true).getState());
                }
                CraftBlock againstBlock = CraftBlock.at(event.getLevel(), event.getPos().relative(direction.getOpposite()));
                ItemStack bukkitStack;
                if (hand == InteractionHand.MAIN_HAND) {
