...
import io.izzel.freshwaterfish.common.mod.server.api.DefaultFreshwaterFishServer;
...
public class FreshwaterFishServer {
    ...
    public static CraftServer createOrLoad(DedicatedServer console, PlayerList playerList) {
        if (server == null) {
            Arclight.setServer(new DefaultFreshwaterFishServer());
            try {
            ...
            try {
                ...
                FreshwaterFishMod.LOGGER.error("registry.error", t);
                throw t;
            }
        } else {
