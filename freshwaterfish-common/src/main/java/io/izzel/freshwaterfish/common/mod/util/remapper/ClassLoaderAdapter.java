...
import io.izzel.freshwaterfish.common.mod.util.remapper.generated.RemappingURLClassLoader;
...
public class ClassLoaderAdapter implements PluginTransformer {
    ...
    public void handleClass(ClassNode node, ClassLoaderRemapper remapper, FreshwaterFishRemapConfig config) {
            ...
            for (AbstractInsnNode insnNode : methodNode.instructions) {
                if (insnNode.getOpcode() == Opcodes.NEW) {
                    ...
                    if (replace != null) {
                        ...
                        FreshwaterFishMod.LOGGER.debug(MARKER, "Found new {}/{} call in {} {}", typeInsnNode.desc, ((MethodInsnNode) next).name + ((MethodInsnNode) next).desc, node.name, methodNode.name + methodNode.desc);
        ...
        if (info == null) return;
        FreshwaterFishMod.LOGGER.debug(MARKER, "Transforming classloader class {}", node.name);
        if (!info.remapping) {
            implementIntf(node);
        }
        for (MethodNode methodNode : node.methods) {
    ...
    private void implementIntf(ClassNode node) {
        FreshwaterFishMod.LOGGER.debug(MARKER, "Implementing RemappingClassLoader for class {}", node.name);
        ...
        MethodNode getRemapper = new MethodNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC, "getRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class)), null, null);
        {
            ...
            list.add(new MethodInsnNode(Opcodes.INVOKESTATIC, Type.getInternalName(FreshwaterFishRemapper.class), "createClassLoaderRemapper", Type.getMethodDescriptor(Type.getType(ClassLoaderRemapper.class), Type.getType(ClassLoader.class)), false));
        ...
        FieldNode remapConfig = new FieldNode(Opcodes.ACC_PRIVATE | Opcodes.ACC_SYNTHETIC, "freshwaterfish$remapConfig", Type.getDescriptor(FreshwaterFishRemapConfig.class), null, null);
        MethodNode getConfig = new MethodNode(Opcodes.ACC_PUBLIC | Opcodes.ACC_SYNTHETIC, "getRemapConfig", Type.getMethodDescriptor(Type.getType(FreshwaterFishRemapConfig.class)), null, null);
        {
            final var config = Type.getInternalName(FreshwaterFishRemapConfig.class);

            /*
             * public FreshwaterFishRemapConfig getConfig() {
             *     if (freshwaterfish$remapConfig != null) {
             *         return freshwaterfish$remapConfig;
             *     }
             *     return freshwaterfish$remapConfig = new FreshwaterFishRemapConfig(RemappingClassLoader.needRemap(this));
             * }
             */
            InsnList list = new InsnList();
