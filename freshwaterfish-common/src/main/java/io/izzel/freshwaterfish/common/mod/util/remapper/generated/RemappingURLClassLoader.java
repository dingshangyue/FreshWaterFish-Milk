package io.izzel.freshwaterfish.common.mod.util.remapper.generated;

import com.google.common.io.ByteStreams;
import io.izzel.freshwaterfish.common.mod.util.remapper.FreshwaterFishRemapConfig;
import io.izzel.freshwaterfish.common.mod.util.remapper.FreshwaterFishRemapper;
import io.izzel.freshwaterfish.common.mod.util.remapper.ClassLoaderRemapper;
import io.izzel.freshwaterfish.common.mod.util.remapper.RemappingClassLoader;
...
public class RemappingURLClassLoader extends URLClassLoader implements RemappingClassLoader {
    ...
    public FreshwaterFishRemapConfig config = new FreshwaterFishRemapConfig(RemappingClassLoader.needRemap(this));
            ...
            try {
                ...
                byteSource = () -> {
                    try (InputStream is = connection.getInputStream()) {
                        ...
                        classBytes = FreshwaterFishRemapper.SWITCH_TABLE_FIXER.apply(classBytes);
                        return classBytes;
                    }
                };
            } catch (IOException e) {
    ...
    private Class<?> tryFallbackLoad(String name) {
        ...
        ClassLoader freshwaterfish = RemappingURLClassLoader.class.getClassLoader();
        if (freshwaterfish != null && freshwaterfish != this && freshwaterfish != parent && freshwaterfish != context) {
            try {
                return Class.forName(name, false, freshwaterfish);
            } catch (ClassNotFoundException ignored) {
            }
        }
        ClassLoader system = ClassLoader.getSystemClassLoader();
        if (system != null && system != this && system != parent && system != context && system != freshwaterfish) {
            try {
    ...
    public ClassLoaderRemapper getRemapper() {
        if (remapper == null) {
            remapper = FreshwaterFishRemapper.createClassLoaderRemapper(this);
        }
        return remapper;
    }

    @Override
    public FreshwaterFishRemapConfig getRemapConfig() {
        return config;
    }
}
