package io.izzel.freshwaterfish.common.mod.server;

import com.google.common.collect.*;
import io.izzel.arclight.api.ArclightVersion;
import io.izzel.arclight.api.EnumHelper;
import io.izzel.arclight.api.Unsafe;
import io.izzel.freshwaterfish.common.bridge.bukkit.EntityTypeBridge;
import io.izzel.freshwaterfish.common.bridge.bukkit.MaterialBridge;
import io.izzel.freshwaterfish.common.bridge.bukkit.SimpleRegistryBridge;
...
import io.izzel.freshwaterfish.common.mod.util.types.FreshwaterFishEnchantment;
import io.izzel.freshwaterfish.common.mod.util.types.FreshwaterFishPotionEffect;
import io.izzel.freshwaterfish.i18n.FreshwaterFishConfig;
import io.izzel.freshwaterfish.i18n.conf.EntityPropertySpec;
...
public class BukkitRegistry {
    ...
    public static void registerAll(DedicatedServer console) {
        CrashReportCallables.registerCrashCallable("FreshwaterFish Release", ArclightVersion.current()::getReleaseName);
        CrashReportCallables.registerCrashCallable("FreshwaterFish", new CraftCrashReport());
        loadMaterials();
        loadPotions();
        loadEnchantments();
        loadEntities();
    ...
    private static void loadFluids() {
        ...
        for (var fluidType : ForgeRegistries.FLUIDS) {
            if (!FLUIDTYPE_FLUID.containsKey(fluidType)) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as fluid {}", key, bukkit);
            }
        }
    ...
    private static void loadCookingBookCategory() {
        ...
        for (CookingBookCategory category : CookingBookCategory.values()) {
            try {
                ...
                newTypes.add(bukkit);
                FreshwaterFishMod.LOGGER.debug("Registered {} as cooking category {}", name, bukkit);
            }
        }
    ...
    private static void loadEndDragonPhase() {
        ...
        for (var id = EnderDragon.Phase.values().length; id < max; id++) {
            ...
            FreshwaterFishMod.LOGGER.debug("Registered {} as ender dragon phase {}", name, newPhase);
        }
    ...
    private static void loadSpawnCategory() {
        ...
        for (var category : MobCategory.values()) {
            try {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as spawn category {}", name, spawnCategory);
            }
        }
    ...
    private static void loadStats() {
        ...
        for (StatType<?> statType : ForgeRegistries.STAT_TYPES) {
            ...
            if (statistic == null) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as stats {}", location, statistic);
                i++;
            }
        }
        for (ResourceLocation location : BuiltInRegistries.CUSTOM_STAT) {
            ...
            if (statistic == null) {
                ...
                newTypes.add(statistic);
                STATS.put(location, statistic);
                FreshwaterFishMod.LOGGER.debug("Registered {} as custom stats {}", location, statistic);
                i++;
            }
        }
    ...
    private static void loadArts() {
        ...
        for (var paintingType : ForgeRegistries.PAINTING_VARIANTS) {
            ...
            if (bukkit == null) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as art {}", location, bukkit);
                i++;
            }
        }
    ...
    private static void loadBiomes(DedicatedServer console) {
        ...
        for (net.minecraft.world.level.biome.Biome biome : registry) {
            ...
            if (bukkit == null) {
                ...
                newTypes.add(bukkit);
                Unsafe.putObject(bukkit, keyOffset, CraftNamespacedKey.fromMinecraft(location));
                FreshwaterFishMod.LOGGER.debug("Registered {} as biome {}", location, bukkit);
            }
        }
        EnumHelper.addEnums(Biome.class, newTypes);
        FreshwaterFishMod.LOGGER.info("registry.biome", newTypes.size());
    }
    ...
    private static void loadVillagerProfessions() {
        ...
        for (VillagerProfession villagerProfession : ForgeRegistries.VILLAGER_PROFESSIONS) {
            ...
            if (profession == null) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as villager profession {}", location, profession);
            }
        }
        EnumHelper.addEnums(Villager.Profession.class, newTypes);
        FreshwaterFishMod.LOGGER.info("registry.villager-profession", newTypes.size());
    }
    ...
    public static void registerEnvironments(Registry<LevelStem> registry) {
        ...
        for (Map.Entry<ResourceKey<LevelStem>, LevelStem> entry : registry.entrySet()) {
            ...
            if (environment == null) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as environment {}", key.location(), environment);
                i++;
            }
        }
        EnumHelper.addEnums(World.Environment.class, newTypes);
        FreshwaterFishMod.LOGGER.info("registry.environment", newTypes.size());
    }
    ...
    private static void loadEntities() {
        ...
        for (net.minecraft.world.entity.EntityType<?> type : ForgeRegistries.ENTITY_TYPES) {
            ...
            if (location.getNamespace().equals(NamespacedKey.MINECRAFT)) {
                ...
                } else FreshwaterFishMod.LOGGER.warn("Cannot find {} in {}", location, EntityType.class);
            }
            if (!found) {
                String name = ResourceLocationUtil.standardize(location);
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as entity {}", location, entityType);
            }
            ENTITY_NAME_MAP.put(location.toString(), entityType);
        }
        EnumHelper.addEnums(EntityType.class, newTypes);
        EntityClassLookup.init();
        FreshwaterFishMod.LOGGER.info("registry.entity-type", newTypes.size());
    }
    ...
    private static void loadEnchantments() {
        ...
        for (net.minecraft.world.item.enchantment.Enchantment enc : ForgeRegistries.ENCHANTMENTS) {
            try {
                ...
                String name = ResourceLocationUtil.standardize(location);
                FreshwaterFishEnchantment enchantment = new FreshwaterFishEnchantment(enc, name);
                Enchantment.registerEnchantment(enchantment);
                FreshwaterFishMod.LOGGER.debug("Registered {} as enchantment {}", location, enchantment);
            } catch (Exception e) {
                FreshwaterFishMod.LOGGER.error("Failed to register enchantment {}: {}", ForgeRegistries.ENCHANTMENTS.getKey(enc), e.getMessage());
            }
        }
        Enchantment.stopAcceptingRegistrations();
        FreshwaterFishMod.LOGGER.info("Registered {} enchantments", size - origin);
    }
    ...
    private static void loadPotions() {
        ...
        for (MobEffect eff : ForgeRegistries.MOB_EFFECTS) {
            try {
                ...
                String name = ResourceLocationUtil.standardize(location);
                FreshwaterFishPotionEffect effect = new FreshwaterFishPotionEffect(eff, name);
                PotionEffectType.registerPotionEffectType(effect);
                FreshwaterFishMod.LOGGER.debug("Registered {} as potion {}", location, effect);
            } catch (Exception e) {
                FreshwaterFishMod.LOGGER.error("Failed to register potion effect {}: {}", ForgeRegistries.MOB_EFFECTS.getKey(eff), e.getMessage());
            }
        }
        PotionEffectType.stopAcceptingRegistrations();
        FreshwaterFishMod.LOGGER.info("Registered {} potion effects", size - origin);
        ...
        for (var potion : ForgeRegistries.POTIONS) {
            ...
            if (CraftPotionUtil.toBukkit(location.toString()).getType() == PotionType.UNCRAFTABLE && potion != Potions.EMPTY) {
                ...
                FreshwaterFishMod.LOGGER.debug("Registered {} as potion type {}", location, potionType);
            }
        }
    ...
    private static void loadMaterials() {
        ...
        for (Block block : ForgeRegistries.BLOCKS) {
            ...
            if (material == null) {
                ...
                i++;
                blocks++;
                FreshwaterFishMod.LOGGER.debug("Registered {} as block {}", location, material);
                list.add(material);
            } else {
        ...
        for (Item item : ForgeRegistries.ITEMS) {
            ...
            if (material == null) {
                ...
                i++;
                items++;
                FreshwaterFishMod.LOGGER.debug("Registered {} as item {}", location, material);
                list.add(material);
            }
        ...
        FreshwaterFishMod.LOGGER.info("registry.material", i - origin, blocks, items);
    }

    private static MaterialPropertySpec matSpec(ResourceLocation location) {
        return FreshwaterFishConfig.spec().getCompat().getMaterial(location.toString()).orElse(MaterialPropertySpec.EMPTY);
    }

    private static EntityPropertySpec entitySpec(ResourceLocation location) {
        return FreshwaterFishConfig.spec().getCompat().getEntity(location.toString()).orElse(EntityPropertySpec.EMPTY);
    }
    ...
    public static Pose toBukkitPose(net.minecraft.world.entity.Pose nms) {
        if (Pose.values().length <= nms.ordinal()) {
            ...
            for (var id = Pose.values().length; id < forgeCount; id++) {
                ...
                newTypes.add(newPhase);
                FreshwaterFishMod.LOGGER.debug("Registered {} as pose {}", name, newPhase);
            }
